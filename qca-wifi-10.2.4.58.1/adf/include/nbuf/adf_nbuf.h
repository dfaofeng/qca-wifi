...
#define adf_nbuf_get_paddr __adf_nbuf_get_paddr_0
...
adf_nbuf_get_frag_vaddr(adf_nbuf_t buf, int frag_num)
{
    ...
    if (buf->mspd_data) {
       return ((frag_num < NBUF_NUM_EXTRA_FRAGS(buf)) ?
               NBUF_EXTRA_FRAG_VADDR(buf, frag_num) : (buf->mspd_data + buf->mspd_ofst));
    } else
#endif
...
#ifdef ATH_USE_NCNB
static inline adf_nbuf_t
adf_nbuf_alloc_ncnb(adf_os_device_t    osdev,
               adf_os_size_t           size,
               int                  reserve,
               int                  align,
               int                  prio)
{
    return __adf_nbuf_alloc_ncnb(osdev, size, reserve,align, prio);
}
#endif
