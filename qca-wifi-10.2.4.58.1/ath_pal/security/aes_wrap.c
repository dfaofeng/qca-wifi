...
int aes_wrap(osdev_t osdev, const u8 *kek, int n, const u8 *plain, u8 *cipher)
{
	...
	ctx = aes_encrypt_init(osdev, kek, 16);
	if (ctx == NULL)
		return -1;
...
int aes_unwrap(osdev_t osdev, const u8 *kek, int n, const u8 *cipher, u8 *plain)
{
	...
	ctx = aes_decrypt_init(osdev, kek, 16);
	if (ctx == NULL)
		return -1;
			 ...
			 const u8 *addr[], const int *len, u8 *mac)
{
	...
	ctx = aes_encrypt_init(osdev, key, 16);
	if (ctx == NULL)
		return -1;
...
{
	return omac1_aes_128_vector(osdev, key, 1, &data, &data_len, mac);
}
			...
			u8 *data, int data_len, u8 *tag)
{
	...
	omac1_aes_128(osdev, key, buf, 16 + nonce_len, nonce_mac);

	buf[15] = 1;
	os_memcpy(buf + 16, hdr, hdr_len);
	omac1_aes_128(osdev, key, buf, 16 + hdr_len, hdr_mac);

	aes_128_ctr_encrypt(osdev, key, nonce_mac, data, data_len);
	buf[15] = 2;
	os_memcpy(buf + 16, data, data_len);
	omac1_aes_128(osdev, key, buf, 16 + data_len, data_mac);
 ...
 * Returns: 0 on success, -1 on failure
 */
int aes_128_cbc_encrypt(osdev_t osdev, const u8 *key, const u8 *iv, u8 *data, int data_len)
{
	void *ctx;
	u8 cbc[BLOCK_SIZE];
	u8 *pos = data;
 ...
 * Returns: 0 on success, -1 on failure
 */
int aes_128_cbc_decrypt(osdev_t osdev, const u8 *key, const u8 *iv, u8 *data, int data_len)
{
	void *ctx;
	...
	ctx = aes_decrypt_init(osdev, key, 16);
	if (ctx == NULL)
		return -1;
	os_memcpy(cbc, iv, BLOCK_SIZE);
