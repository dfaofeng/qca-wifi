 ...
 * @mac: Buffer for the hash (16 bytes)
 */
void pal_hmac_md5(const u8 *key, size_t key_len, const u8 *data, size_t data_len,
	      u8 *mac)
{
	hmac_md5_vector(key, key_len, 1, &data, &data_len, mac);
}

void HMAC_MD5(u8* Key, u8* Dest, u32 *val, u8 Len)
{
	pal_hmac_md5((const u8*)Key, (size_t)Len, (const u8 *)val, 4, Dest);
}
 ...
 * of bytes.
 */
int MD5_Update( MD5_CTX *ctx, unsigned char const *buf, size_t len)
{
    u32 t;
 ...
 */
int MD5_Final(unsigned char digest[16], MD5_CTX *ctx)
{
    unsigned count;
    unsigned char *p;
