...
extern void hmac_md5_vector(const u8 *key, size_t key_len, size_t num_elem,
		     const u8 *addr[], const size_t *len, u8 *mac);

extern void pal_hmac_md5(const u8 *key, size_t key_len, const u8 *data, size_t data_len,
	      u8 *mac);
		...
		const u8 *seed, size_t seed_len, u8 *buf, size_t buf_len)
{
	...
	output_len[0] = (u8)((buf_len >> 8) & 0xff);
	output_len[1] = (u8)(buf_len & 0xff);
	pos = 0;
	while (pos < buf_len) {
		counter++;
		plen = buf_len - pos;
	    ...
	    const u8 *seed, size_t seed_len, u8 *out, size_t outlen)
{
	...
	for (i = 0; i < outlen; i++) {
		if (MD5_pos == MD5_MAC_LEN) {
			...
			MD5_pos = 0;
			pal_hmac_md5(S1, L_S1, A_MD5, MD5_MAC_LEN, A_MD5);
		}
		if (SHA1_pos == SHA1_MAC_LEN) {
			hmac_sha1_vector(S2, L_S2, 3, SHA1_addr, SHA1_len,
					 P_SHA1);
			SHA1_pos = 0;
			pal_hmac_sha1(S2, L_S2, A_SHA1, SHA1_MAC_LEN, A_SHA1);
		}
			  ...
			  u8 *digest)
{
	...
	for (i = 1; i < iterations; i++) {
		pal_hmac_sha1((u8 *) passphrase, passphrase_len, tmp, SHA1_MAC_LEN,
			  tmp2);
