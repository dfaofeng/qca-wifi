 ...
 */
#ifndef AR956X_EMULATION
static int Ar9300_eeprom_template_preference = ar9300_eeprom_template_generic;
#else
static int Ar9300_eeprom_template_preference = ar9300_eeprom_template_aphrodite;
#endif
...
ar9300_eeprom_write(struct ath_hal *ah, u_int off, u_int16_t data)
{
    ...
    u_int32_t eeprom_write_gpio =
        ahp->ah_eeprom.base_eep_header.eeprom_write_enable_gpio;
    ...
    u_int32_t config_bit_position = 2 * eeprom_write_gpio;
...
ar9300_otp_read(struct ath_hal *ah, u_int off, u_int32_t *data, bool is_wifi)
{
    ...
    if (AR_SREV_DRAGONFLY(ah) || AR_SREV_HONEYBEE(ah)){ /* no OTP for Dragonfly nad honeybee */
        return false;
    }
...
ar9300_eeprom_attach(struct ath_hal *ah)
{
...
#ifdef __NetBSD__
    ar9300_eeprom_template_preference(ar9300_eeprom_template_osprey_k31);
#endif
    ...
    if (1) { /* always true but avoids compile warning re unreachable code */
        ...
        int mdata_size = ar9300_eeprom_struct_size();
        ar9300_eeprom_restore_something(ah, mptr, mdata_size);
        ahp->ah_eeprom.mac_addr[0] = 0x00;
        ahp->ah_eeprom.mac_addr[1] = 0x03;
        ahp->ah_eeprom.mac_addr[2] = 0x7F;
...
/*#ifdef MDK_AP*/ /* MDK_AP is defined only in NART AP build */
    ...
    if (AR_SREV_HORNET(ah) || AR_SREV_WASP(ah) || AR_SREV_SCORPION(ah)
        || AR_SREV_HONEYBEE(ah) || AR_SREV_DRAGONFLY(ah)) {
        ahp->try_eeprom = 0;
    }
...
bool ar9300_xpa_timing_control_apply(struct ath_hal *ah, bool is_2ghz)
{
    ...
    if ((eep->base_eep_header.feature_enable & 0x80) >> 7) {
		if (AR_SREV_OSPREY(ah) || AR_SREV_AR9580(ah) || AR_SREV_WASP(ah) || AR_SREV_HONEYBEE(ah) || AR_SREV_DRAGONFLY(ah)) {
			if (is_2ghz) {
