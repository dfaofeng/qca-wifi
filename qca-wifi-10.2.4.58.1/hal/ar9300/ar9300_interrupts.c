            ...
            p_cap->hal_mci_support)
        {
            ...
            if ((int_raw == 0xdeadbeef) || (int_rx_msg == 0xdeadbeef))
            {
                ...
                *masked |= HAL_INT_MCI;
                ...
                HDPRINTF(ah, HAL_DBG_INTERRUPT, "%s:AR_INTR_SYNC_MCI\n", __func__);
            }
        }
#endif
    }
...
void
ar9300_disable_interrupts_on_exit(struct ath_hal *ah)
{
    OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_INTR_ASYNC_ENABLE), 0);
    OS_REG_WRITE(ah, AR_HOSTIF_REG(ah, AR_INTR_ASYNC_MASK), 0);
 ...
 */
HAL_INT
ar9300_set_interrupts(struct ath_hal *ah, HAL_INT ints, bool nortc)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
