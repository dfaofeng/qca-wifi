...
static void ar9300_mci_print_msg(struct ath_hal *ah, bool send,u_int8_t hdr,
                                 int len, u_int32_t *pl)
{
    ...
    HDPRINTF(ah, HAL_DBG_BT_COEX, "%s\n", s);
/*
    for ( i=0; i<(len + 3)/4; i++)
    {
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI)   0x%08x\n", *(pl + i));
    }
*/
#endif
}
...
static
void ar9300_mci_osla_setup(struct ath_hal *ah, bool enable)
{
    ...
    if (enable) {
            ...
            ATH_MCI_CONFIG_DISABLE_AGGR_THRESH))
        {
            ...
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) SCHED aggr thresh: off\n");
        }
        OS_REG_RMW_FIELD(ah, AR_BTCOEX_CTRL,
                         AR_BTCOEX_CTRL_ONE_STEP_LOOK_AHEAD_EN, 1);
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) SCHED one step look ahead: on\n");
    }
    else {
        OS_REG_CLR_BIT(ah, AR_BTCOEX_CTRL, 
            AR_BTCOEX_CTRL_ONE_STEP_LOOK_AHEAD_EN);
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) SCHED one step look ahead: off\n");
    }
}
...
void ar9300_mci_remote_reset(struct ath_hal *ah, bool wait_done)
{
...
void ar9300_mci_send_lna_transfer(struct ath_hal *ah, bool wait_done)
{
...
static void ar9300_mci_send_req_wake(struct ath_hal *ah, bool wait_done)
{
    ar9300_mci_send_message(ah, MCI_REQ_WAKE, 
        HAL_MCI_FLAG_DISABLE_TIMESTAMP, AH_NULL, 0, wait_done, false);

    OS_DELAY(5);
}

void ar9300_mci_send_sys_waking(struct ath_hal *ah, bool wait_done)
{
    ar9300_mci_send_message(ah, MCI_SYS_WAKING, 
        HAL_MCI_FLAG_DISABLE_TIMESTAMP, AH_NULL, 0, wait_done, false);
}

static void ar9300_mci_send_lna_take(struct ath_hal *ah, bool wait_done)
{
    u_int32_t payload = 0x70000000;

    /* LNA gain index is set to 7. */
    ar9300_mci_send_message(ah, MCI_LNA_TAKE, 
        HAL_MCI_FLAG_DISABLE_TIMESTAMP, &payload, 1, wait_done, false);
}

static void ar9300_mci_send_sys_sleeping(struct ath_hal *ah, bool wait_done)
{
...
static void
ar9300_mci_send_coex_version_query(struct ath_hal *ah, bool wait_done)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    u_int32_t payload[4] = {0, 0, 0, 0};

    if ((ahp->ah_mci_coex_bt_version_known == false) &&
        (ahp->ah_mci_bt_state != MCI_BT_SLEEP)) {
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) Send Coex version query.\n");
        MCI_GPM_SET_TYPE_OPCODE(payload,
            MCI_GPM_COEX_AGENT, MCI_GPM_COEX_VERSION_QUERY);
        ar9300_mci_send_message(ah, MCI_GPM, 0, payload, 16, wait_done, true);
    }
}

static void
ar9300_mci_send_coex_version_response(struct ath_hal *ah, bool wait_done)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    u_int32_t payload[4] = {0, 0, 0, 0};

    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) Send Coex version response.\n");
    MCI_GPM_SET_TYPE_OPCODE(payload,
...
static void
ar9300_mci_send_coex_wlan_channels(struct ath_hal *ah, bool wait_done)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    u_int32_t *payload = &ahp->ah_mci_coex_wlan_channels[0];

    if ((ahp->ah_mci_coex_wlan_channels_update == true) &&
        (ahp->ah_mci_bt_state != MCI_BT_SLEEP))
    {
        MCI_GPM_SET_TYPE_OPCODE(payload,
...
static void ar9300_mci_send_coex_bt_status_query(struct ath_hal *ah,
                                    bool wait_done, u_int8_t query_type)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    ...
    if (ahp->ah_mci_bt_state != MCI_BT_SLEEP) {
        ...
        *(((u_int8_t *)pld) + MCI_GPM_COEX_B_BT_BITMAP) = query_type;
         ...
         */
        if (!ar9300_mci_send_message(ah, MCI_GPM, 0, pld, 16, wait_done, true))
        {
            if (query_btinfo) {
...
static bool ar9300_mci_send_coex_bt_flags(struct ath_hal *ah, bool wait_done,
                                          u_int8_t opcode, u_int32_t bt_flags)
{
    ...
    *(((u_int8_t *)pld) + MCI_GPM_COEX_W_BT_FLAGS + 3) =
    ...
    return ar9300_mci_send_message(ah, MCI_GPM, 0, pld, 16, wait_done, true);
}

void ar9300_mci_2g5g_changed(struct ath_hal *ah, bool is_2g)
{
    struct ath_hal_9300 *ahp = AH9300(ah);

    if (ahp->ah_mci_coex_2g5g_update == false) {
        if (ahp->ah_mci_coex_is_2g == is_2g) {
            //HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) BT_MCI_FLAGS: not changed\n");
        } else {
            ahp->ah_mci_coex_2g5g_update = true;
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) BT_MCI_FLAGS: changed\n");
        }
    } else {
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) BT_MCI_FLAGS: force send\n");
    }
    ahp->ah_mci_coex_is_2g = is_2g;
}

static void ar9300_mci_send_2g5g_status(struct ath_hal *ah, bool wait_done)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    ...
    if (AR_SREV_JUPITER_10(ah) && (ahp->ah_mci_bt_state != MCI_BT_SLEEP)) {
        ahp->ah_mci_coex_2g5g_update = false;
    }
}

void ar9300_mci_2g5g_switch(struct ath_hal *ah, bool wait_done)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    ...
    {
        if (ahp->ah_mci_coex_is_2g) {
            ar9300_mci_send_2g5g_status(ah, true);

            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) Send LNA trans\n");
            ...
            if (AR_SREV_JUPITER_20(ah) || AR_SREV_APHRODITE(ah)) {
                ...
                {
                    ar9300_mci_osla_setup(ah, true);
                }
            }
        } else {
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) Send LNA take\n");
            ar9300_mci_send_lna_take(ah, true);
            OS_DELAY(5);
            ...
            if (AR_SREV_JUPITER_20(ah) || AR_SREV_APHRODITE(ah)) {
                OS_REG_SET_BIT(ah, AR_GLB_CONTROL,
                    AR_BTCOEX_CTRL_BT_OWN_SPDT_CTRL);
                ar9300_mci_osla_setup(ah, false);
            }
    ...
    if (AR_SREV_JUPITER(ah)) {
        if (ahp->ah_mci_coex_is_2g) {
            ahp->ah_reduced_self_gen_mask = true;
            OS_REG_WRITE(ah, AR_SELFGEN_MASK, 0x02);
            ar9300_txbf_set_basic_set(ah);
        }
        else {
            ahp->ah_reduced_self_gen_mask = false;
            ar9300_txbf_set_basic_set(ah);
        }
    }
}
...
void ar9300_mci_mute_bt(struct ath_hal *ah)
{
    ...
    ar9300_mci_send_lna_take(ah, true);
    OS_DELAY(5);
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) Send sys sleeping\n");
    ar9300_mci_send_sys_sleeping(ah, true);
}
...
static void ar9300_mci_observation_set_up(struct ath_hal *ah)
{
    ...
    */
    ...
    if (AR_SREV_DRAGONFLY(ah)) {		
		OS_REG_RMW_FIELD(ah, AR_PHY_TEST_CTL_STATUS_DRAGONFLY, 
        ...
        case MCI_GPM_COEX_AGENT:
            switch (gpm_opcode)
            {
                case MCI_GPM_COEX_VERSION_QUERY:
                    ...
                    ar9300_mci_send_coex_version_response(ah, true);
                    break;
                ...
                case MCI_GPM_COEX_VERSION_RESPONSE:
                    ...
                    ahp->ah_mci_coex_bt_version_known = true;
                    HDPRINTF(ah, HAL_DBG_BT_COEX,
                ...
                case MCI_GPM_COEX_STATUS_QUERY:
                    ...
                    {
                        ahp->ah_mci_coex_wlan_channels_update = true;
                        ar9300_mci_send_coex_wlan_channels(ah, true);
                    }
                    break;
                                  ...
                                  u_int8_t gpm_opcode, int32_t time_out)
{
    ...
    while (time_out > 0)
    {
        ...
        if (time_out) {
            ...
            ar9300_mci_print_msg(ah, false, MCI_GPM, 16, p_gpm);
    ...
    while (more_data == HAL_MCI_GPM_MORE) {
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) discard remaining GPM\n");
        offset = ar9300_mci_state(ah,
            HAL_MCI_STATE_NEXT_GPM_OFFSET, &more_data);
...
static void ar9300_mci_prep_interface(struct ath_hal *ah)
{
    ...
    /* Remote Reset */
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) %s: Reset sequence start\n", __func__);
    ...
    /* Send REQ_WAKE to BT */
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) %s: Send REQ_WAKE to remote(BT)\n",
        __func__);

    ar9300_mci_send_req_wake(ah, true);
        ...
        AR_MCI_INTERRUPT_RX_MSG_SYS_WAKING, 500))
    {
        ...
        if (AR_SREV_JUPITER_10(ah) || ahp->ah_mci_coex_is_2g) {
            /* Send LNA_TRANS */
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) %s: Send LNA_TRANS to BT\n", 
                __func__);
            ar9300_mci_send_lna_transfer(ah, true);
                      ...
                      u_int32_t sched_addr)
{
    ...
    ar9300_mci_reset(ah, true, true, true);
}
...
void ar9300_mci_reset(struct ath_hal *ah, bool en_int, bool is_2g,
                      bool is_full_sleep)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    ...
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) %s: full_sleep = %d, is_2g = %d\n",
    ...
    if (ahpriv->ah_config.ath_hal_mci_config & ATH_MCI_CONFIG_CONCUR_TX) {
             ...
             ATH_MCI_CONFIG_CONCUR_TX) == ATH_MCI_CONCUR_TX_SHARED_CHN)
        {
            ahp->ah_mci_concur_tx_en = (ahp->ah_bt_coex_flag & 
                HAL_BT_COEX_FLAG_MCI_MAX_TX_PWR) ? true : false;

            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) concur_tx_en = %d\n", 
                ...
                case MCI_GPM_COEX_HALT_BT_GPM:
                        ...
                        MCI_GPM_COEX_BT_GPM_UNHALT)
                    {
                        ...
                        if (queue == true) {
                            ...
                            ahp->ah_mci_halted_bt_gpm = false;
                            HDPRINTF(ah, HAL_DBG_BT_COEX,
...
bool ar9300_mci_send_message(struct ath_hal *ah, u_int8_t header,
                              u_int32_t flag, u_int32_t *payload, 
                              u_int8_t len, bool wait_done, bool check_bt)
{
    int i;
    struct ath_hal_9300 *ahp = AH9300(ah);
    bool msg_sent = false;
    ...
    if ((regval == 0xdeadbeef) || !(regval & AR_BTCOEX_CTRL_MCI_MODE_EN)) {
        ...
        ar9300_mci_queue_unsent_gpm(ah, header, payload, true);
        return false;
    }
    ...
    ar9300_mci_print_msg(ah, true, header, len, payload);
                                    ...
                                    AR_MCI_INTERRUPT_SW_MSG_DONE, 500) == 0)
    {
        ...
        ar9300_mci_queue_unsent_gpm(ah, header, payload, false);
        msg_sent = true;
    }
...
void ar9300_mci_sync_bt_state(struct ath_hal *ah)
{
    ...
    if (ahp->ah_mci_bt_state != MCI_BT_SLEEP) {
        ...
        ar9300_mci_send_coex_version_query(ah, true);
#endif
        ar9300_mci_send_coex_wlan_channels(ah, true);
        if (ahp->ah_mci_unhalt_bt_gpm == true) {
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) %s: UNHALT BT GPM\n", __func__);
            ar9300_mci_send_coex_halt_bt_gpm(ah, false, true);
        }
    }
}

static bool ar9300_mci_is_gpm_valid(struct ath_hal *ah, u_int32_t msg_index)
{
    struct ath_hal_9300 *ahp = AH9300(ah);
    u_int32_t *payload;
    ...
    if (recv_type == MCI_GPM_RSVD_PATTERN) {
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) Skip RSVD GPM\n");
        return false;
    }
...
ar9300_mci_state(struct ath_hal *ah, u_int32_t state_type, u_int32_t *p_data)
{
    ...
    case HAL_MCI_STATE_LAST_SCHD_MSG_OFFSET:
...
#if AH_MCI_DEBUG_PRINT_SCHED
        {
            ...
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) SCHED\n");
            HDPRINTF(ah, HAL_DBG_BT_COEX,
            ...
            for (sched_idx = prev_index; sched_idx <= index; sched_idx++) {
                ...
                ar9300_mci_print_msg(ah, false, MCI_SCHD_INFO, 16, pld);
                HDPRINTF(ah, HAL_DBG_BT_COEX,
    ...
    case HAL_MCI_STATE_REMOTE_SLEEP:
        ...
        case HAL_MCI_STATE_SET_BT_COEX_VERSION:
            if (p_data == NULL) {
                ...
                ahp->ah_mci_coex_bt_version_known = true;
                HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) BT version set: %d.%d\n",
                        ahp->ah_mci_coex_major_version_bt,
                        ahp->ah_mci_coex_minor_version_bt);
            }
            break;
        ...
        case HAL_MCI_STATE_SEND_STATUS_QUERY:
            if (AR_SREV_JUPITER_10(ah)) {
                ar9300_mci_send_coex_bt_status_query(ah, true,
        ...
        case HAL_MCI_STATE_NEED_FLUSH_BT_INFO:
             ...
             * out successfully.
             *
             * Checking (ah_mci_unhalt_bt_gpm == false) instead of checking
             * (ahp->ah_mci_halted_bt_gpm == false) will make sure currently is
             * in UNHALT-ed mode and BT can respond to status query.
             */
            if ((ahp->ah_mci_unhalt_bt_gpm == false) &&
                (ahp->ah_mci_need_flush_btinfo == true))
            {
                value = 1;
            }
        ...
        case HAL_MCI_STATE_RECOVER_RX:
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) hal RECOVER_RX\n");
            ar9300_mci_prep_interface(ah);
            ahp->ah_mci_query_bt = true;
            ahp->ah_mci_need_flush_btinfo = true;
            ar9300_mci_send_coex_wlan_channels(ah, true);
        ...
        case HAL_MCI_STATE_DEBUG:
            if (p_data != NULL) {
                if (*p_data == HAL_MCI_STATE_DEBUG_REQ_BT_DEBUG) {
                    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) QUERY_BT_DEBUG\n");
                    ar9300_mci_send_coex_bt_status_query(ah, true,
                        MCI_GPM_COEX_QUERY_BT_DEBUG);
                    OS_DELAY(10);
                    if (AR_SREV_JUPITER_20(ah) || AR_SREV_APHRODITE(ah)) {
                        ar9300_mci_send_coex_bt_flags(ah, true,
...
void ar9300_mci_detach(struct ath_hal *ah)
{
    ...
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(MCI) ar9300_mci_detach\n");
