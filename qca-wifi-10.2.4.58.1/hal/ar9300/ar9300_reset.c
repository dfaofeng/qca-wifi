...
static void
ar9300_update_cca_threshold(struct ath_hal *ah, int16_t nfarray[NUM_NF_READINGS])
{
    struct ath_hal_private *ahpriv = AH_PRIVATE(ah);
    int chan, chain;
    ...
    u_int32_t *coef_mantissa, u_int32_t *coef_exponent)
{
    ...
    /*
     * This is trying to avoid the case that with a scaled down full system 
     * emulation, the clock speed it much lower than real chip. The output 
     * of the calculation might be out of the range for the designated 
     * register. We have such problem during Jupiter emulation test.
     */
...
#ifdef AR9300_EMULATION
    HDPRINTF(AH_NULL, HAL_DBG_UNMASKABLE, "FIXME: ar9300_init_pll\n");
#ifdef AR9300_EMULATION_BB
    ...
    if (do_nf_cal) {
        ...
        if (nf_done) {
            ...
            if (AH_PRIVATE(ah)->ah_config.ath_hal_enable_adaptiveCCAThres) {
                ar9300_update_cca_threshold(ah, nf_buf);
            }
    ...
    HAL_CHANNEL_INTERNAL *ichan, HAL_HT_MACMODE macmode)
{
...
#ifdef JUPITER_EMULATION
    {
        ...
        val2 = OS_REG_READ(ah, AR_MAC_PCU_MISC_MODE3);
        printk("%s: PCU_MISC_MODE3=0x%08x\n", __func__,  val2);
...
#undef REG_WRITE
    ...
    if (AR_SREV_JUPITER(ah) || AR_SREV_APHRODITE(ah)) {
        ar9300_prog_ini(ah, &ahp->ah_ini_tx_gain_emu, modes_index);
    }
#endif
...
// Dynamic OSDAC selection only for Scorpion 2GHz . To fix EV#130174
    ...
    if(!AR_SREV_SCORPION(ah)) {
		...
		/*This is true only for Honeybee */
...
#if defined(AR9300_EMULATION_BB) && !defined(AR9550_EMULATION)
...
#ifdef JUPITER_EMULATION
            if (save_full_sleep && (ahp->ah_mci_bt_state == MCI_BT_AWAKE)) {
                u_int32_t debug_flag = HAL_MCI_STATE_DEBUG_REQ_BT_DEBUG;
                ar9300_mci_state(ah, HAL_MCI_STATE_DEBUG, &debug_flag);
            }
#endif
        }
#endif
...
#if !(defined(ART_BUILD)) && (ATH_SUPPORT_LED)
#define REG_WRITE(_reg, _val)   *((volatile u_int32_t *)(_reg)) = (_val);
#define REG_READ(_reg)          *((volatile u_int32_t *)(_reg))
...
#undef REG_WRITE
    ...
    {
        /* 
         * Fixed TSF timer inaccurately issue on emulation
         * (faster than real chip)
        ...
        OS_REG_WRITE(ah, AR_SLP32_INC, 0x2aaa);
#endif
    }
#endif
...
ar9300_crdc_activate(struct ath_hal *ah, int rssi_diff, int enable)
{
    ...
    HDPRINTF(ah, HAL_DBG_CALIBRATE, "diff: %02d comp: %02d reg: %08x %08x\n", 
...
void ar9300_chain_rssi_diff_compensation(struct ath_hal *ah)
{
    ...
    HDPRINTF(ah, HAL_DBG_CALIBRATE, "crdc: avg: %02d %02d ", 
