...
typedef struct osprey_BaseExtension_1 {
	...
	u_int8_t  misc_enable; //misc_enable : bit0 - Tx Gain Cap enable
                           // bit1 - uncompressed Checksum Enable
                           // bit 2 &3 - MinCCA PWR Enable 2G and 5G
...
typedef struct ospreyModalEepHeader {
    ...
    u_int8_t   obdboffst_aux1[OSPREY_MAX_CHAINS];       // 3    //obdboffst2G_aux1 for dragonfly (0x16044/0x16444/0x16844, 5:3)
    u_int8_t   obdboffst_aux2[OSPREY_MAX_CHAINS];       // 3    //obdboffst2G_aux2 for dragonfly (0x16044/0x16444/0x16844, 2:0)
...
#define AR9300_EEPROM_START_ADDR    0x1f011000
#else
...
fbin2freq(u_int8_t fbin, bool is_2ghz)
{
    /*
...
extern bool ar9300_eeprom_restore(struct ath_hal *ah);
...
extern bool ar9300_eeprom_set_power_per_rate_table(struct ath_hal *ah,
...
extern u_int8_t ar9300_eeprom_get_legacy_trgt_pwr(struct ath_hal *ah, u_int16_t rate_index, u_int16_t freq, bool is_2ghz);
extern u_int8_t ar9300_eeprom_get_ht20_trgt_pwr(struct ath_hal *ah, u_int16_t rate_index, u_int16_t freq, bool is_2ghz);
extern u_int8_t ar9300_eeprom_get_ht40_trgt_pwr(struct ath_hal *ah, u_int16_t rate_index, u_int16_t freq, bool is_2ghz);
extern u_int8_t ar9300_eeprom_get_cck_trgt_pwr(struct ath_hal *ah, u_int16_t rate_index, u_int16_t freq);
extern bool ar9300_internal_regulator_apply(struct ath_hal *ah);
extern bool ar9300_drive_strength_apply(struct ath_hal *ah);
extern bool ar9300_attenuation_apply(struct ath_hal *ah, u_int16_t channel);
extern int32_t ar9300_thermometer_get(struct ath_hal *ah);
extern bool ar9300_thermometer_apply(struct ath_hal *ah);
extern bool ar9300_xpa_timing_control_apply(struct ath_hal *ah, bool is_2ghz);
extern bool ar9300_x_lNA_bias_strength_apply(struct ath_hal *ah, bool is_2ghz);
...
extern int32_t ar9300_xpa_bias_level_get(struct ath_hal *ah, bool is_2ghz);
extern bool ar9300_xpa_bias_level_apply(struct ath_hal *ah, bool is_2ghz);
extern u_int32_t ar9300_ant_ctrl_common_get(struct ath_hal *ah, bool is_2ghz);
extern u_int32_t ar9300_ant_ctrl_common2_get(struct ath_hal *ah, bool is_2ghz);
extern u_int16_t ar9300_ant_ctrl_chain_get(struct ath_hal *ah, int chain, bool is_2ghz);
extern bool ar9300_ant_ctrl_apply(struct ath_hal *ah, bool is_2ghz);
/* since valid noise floor values are negative, returns 1 on error */
extern int32_t ar9300_noise_floor_cal_or_power_get(
    struct ath_hal *ah, int32_t frequency, int32_t ichain, bool use_cal);
...
extern bool ar9300_calibration_data_read_flash(struct ath_hal *ah, long address, u_int8_t *buffer, int many);
extern bool ar9300_calibration_data_read_eeprom(struct ath_hal *ah, long address, u_int8_t *buffer, int many);
extern bool ar9300_calibration_data_read_otp(struct ath_hal *ah, long address, u_int8_t *buffer, int many, bool is_wifi);
extern bool ar9300_calibration_data_read(struct ath_hal *ah, long address, u_int8_t *buffer, int many);
