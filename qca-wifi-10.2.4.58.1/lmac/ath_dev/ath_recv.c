...
#ifdef __NetBSD__
        if (!err) {
            OS_SET_TIMER(&sc->rchk_timer, ATH_RX_HANG_CHECK_MS);
        }
#endif
        return err;
 }
        ...
        ieee80211_rx_status_t *rx_status, u_int8_t *chainreset)
{
     ...
     */
    {
        ...
        if (ath_green_ap_sc_get_enable_print(sc->sc_ieee)) {
            if (!(print_now & 0xff)) {
                  adf_os_print("Rx rssi0 %d rssi1 %d rssi2 %d\n", 
...
#define PA2DESC(_sc, _pa)                                               \
    ...
    do {
        ...
        if (wbuf->mspd_data) {       
            int type = -1, subtype, len;
            int total_len = rxs->rs_datalen + sc->sc_rxstatuslen;

            wh = (struct ieee80211_frame *)(wbuf_raw_data(wbuf) + sc->sc_rxstatuslen);
            type = wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK;
            subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;

            if ((type == IEEE80211_FC0_TYPE_DATA) && ((subtype == IEEE80211_FC0_SUBTYPE_DATA) || (subtype == IEEE80211_FC0_SUBTYPE_QOS))) {
                len =  total_len - wbuf->mspd_ofst;
                wbuf->mspd_len = (len >= 0) ? len:0;
            } else {
                if (wbuf->mspd_ofst < total_len)
                    memcpy(wbuf->data + wbuf->mspd_ofst, wbuf->mspd_data + wbuf->mspd_ofst, total_len - wbuf->mspd_ofst);
                wbuf->mspd_len = 0;
            }
        }
#endif
