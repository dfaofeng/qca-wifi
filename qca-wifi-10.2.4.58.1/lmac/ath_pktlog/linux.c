...
{
    int ret, enable;
    ath_generic_softc_handle scn = (ath_generic_softc_handle) ctl->extra1;
    ...
    if (write) {
        ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
                                       lenp, ppos);
        if (ret == 0) {
            printk("Enable in %s:%d\n", __FUNCTION__, enable);
	     if (pl_dev){	
            	  printk("pl_dev:%d; pl_funcs:%d; pktlog_enable:%d; scn:%d\n",
                                        (int)pl_dev, (int)pl_dev->pl_funcs,
                                        (int)pl_dev->pl_funcs->pktlog_enable,
                                        (int)pl_dev->scn);
            	  return pl_dev->pl_funcs->pktlog_enable(scn, enable);
	    }
        }
        else
        ...
        if ((dev = dev_get_by_name(dev_name)) != NULL) {
            ...
            if(pl_dev && pl_dev->tgt_pktlog_enabled) { 
                if(pl_dev->pl_funcs->pktlog_enable(scn, 0)) {
                    printk("Unable to Disable pktlog in the target\n");
                }
            }
...
pktlog_attach(ath_generic_softc_handle scn)
{
    ...
    pl_info_lnx->sysctl_header = NULL;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
	proc_entry = proc_create(proc_name, PKTLOG_PROC_PERM, g_pktlog_pde, &pktlog_fops);
#else
