...
struct CE_handle *CE_init(struct ath_hif_pci_softc *sc,
                          unsigned int CE_id,
                          struct CE_attr *attr);
...
void CE_pause(struct CE_handle *copyeng);

/*==================CE Interrupt Handlers=====================================*/
void CE_per_engine_service_any(int irq, void *arg);
void CE_per_engine_service(struct ath_hif_pci_softc *sc, unsigned int CE_id);
void CE_per_engine_servicereap(struct ath_hif_pci_softc *sc, unsigned int CE_id);

#if QCA_OL_11AC_FAST_PATH
void CE_per_engine_service_each(int irq, struct ath_hif_pci_softc *sc);
void CE_per_engine_service_one(struct ath_hif_pci_softc *sc, uint32_t ce_id);
#endif /* QCA_OL_11AC_FAST_PATH */

/* CE_attr.flags values */
#define CE_ATTR_NO_SNOOP                1  /* Use NonSnooping PCIe accesses? */
#define CE_ATTR_BYTE_SWAP_DATA          2  /* Byte swap data words */
#define CE_ATTR_SWIZZLE_DESCRIPTORS     4  /* Swizzle descriptors? */
#define CE_ATTR_DISABLE_INTR            8  /* no interrupt on copy completion */
...
extern int
CE_send_single(struct CE_handle *ce_tx_hdl,
        adf_nbuf_t msdu,
        uint32_t transfer_id,
        uint32_t len );
