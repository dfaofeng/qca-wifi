...
static void
htt_t2h_stats_pdev_tidq_stats_print(struct wal_dbg_tidq_stats *wal_pdev_tidq_stats, int concise)
{
    struct wal_dbg_txq_stats *tidq_st = &wal_pdev_tidq_stats->txq_st;
    int i = 0;
    ...
    struct wal_dbg_stats *wal_pdev_stats, int concise)
{
    ...
    struct wal_dbg_mem_stats *mem = &wal_pdev_stats->mem;
...
htt_t2h_stats_print(u_int8_t *stats_data, int concise)
{
    ...
    case HTT_DBG_STATS_TIDQ:
        {
            struct wal_dbg_tidq_stats *wal_dbg_tidq_stats_ptr;

            wal_dbg_tidq_stats_ptr = (struct wal_dbg_tidq_stats *)(msg_word + 1);
            htt_t2h_stats_pdev_tidq_stats_print(wal_dbg_tidq_stats_ptr, concise);
            break;
        }
