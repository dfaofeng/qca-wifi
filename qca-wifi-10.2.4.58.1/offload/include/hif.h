...
extern void hif_register_tbl_attach(A_UINT32 hif_type);
...
#define DIAG_TRANSFER_LIMIT 2048 /* maximum number of bytes that can be
...
int HIFDiagWriteMem(HIF_DEVICE *hif_device, A_UINT32 address, A_UINT8 *data, int nbytes);
    ...
    /*
     * Set the hif_started flag in sc, for use by data path
     */
#if QCA_OL_11AC_FAST_PATH
void hif_start_set(HIF_DEVICE *hif_dev);
#endif
...
{\
  if(_hif_ecode==HIF_PIPE_NO_RESOURCE)(_hif_state->sc->scn->pkt_stats.hif_pipe_no_resrc_count)+=1;\
}
/*
 ...
 */
//#define TARGID_TO_PCI_ADDR(targid) (*((void __iomem **)(targid)))
...
u_int32_t sim_target_register_read(struct ol_ath_softc_net80211 *scn, u_int32_t addr);
void sim_target_register_write(struct ol_ath_softc_net80211 *scn, u_int32_t addr, u_int32_t val);
