...
** to the ATH object, normally instantiated as radioX, where X is the
** instance number (e.g. radio0, radio1).
**
...
#if WLAN_PEER_DEBUG_CMD_SUPPORT
static int ol_ath_iw_node_debug_param(struct net_device *dev, struct iw_request_info *info, void *w, char *extra);
#endif
...
#define RTT_TEST
#ifdef RTT_TEST
extern void ol_ath_rtt_meas_req(wmi_unified_t  wmi_handle, struct ieee80211com *ic);
#endif

#if QCA_AIRTIME_FAIRNESS
extern  int wmi_unified_pdev_set_param(wmi_unified_t wmi_handle,WMI_PDEV_PARAM param_id, u_int32_t param_value);
#endif
...
#if WLAN_PEER_DEBUG_CMD_SUPPORT
#define ATH_IOCTL_NODE_DBG_PARAM    (SIOCIWFIRSTPRIV+13)
#endif

#if QCA_AIRTIME_FAIRNESS
#define ATH_IOCTL_SET_ATF_SCHED_DUR            SIOCIWFIRSTPRIV+14
#endif

#if WLAN_DEBUG_CMD_SUPPORT
#define ATH_IOCTL_GET_PDEV_INFO     (SIOCIWFIRSTPRIV+15)
#define ATH_IOCTL_GET_VDEV_INFO     (SIOCIWFIRSTPRIV+16)
#endif
...
#if QCA_AIRTIME_FAIRNESS
static int ol_ath_iw_set_atf_sched_dur(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
#define OL_ATF_SCHED_DUR_AC 0x03
#define OL_ATF_SCHED_DURATION 0x3fffffff    
    ...
    retval = wmi_unified_pdev_set_param(scn->wmi_handle, WMI_PDEV_PARAM_ATF_SCHED_DURATION,
            (ac & OL_ATF_SCHED_DUR_AC) << 30 | (OL_ATF_SCHED_DURATION & duration));

    return retval;
#undef OL_ATF_SCHED_DUR_AC
#undef OL_ATF_SCHED_DURATION    
}
#endif
...
enum {
    ...
    OL_SPECIAL_PARAM_ENABLE_MAC_REQ = 14,
};
...
static const struct iw_priv_args ol_ath_iw_priv_args[] = {
   ...
   */
    ...
    */
    ...
    /* Global Rx decapsulation mode of the target */
    { OL_ATH_PARAM_RX_DECAP_MODE | OL_ATH_PARAM_SHIFT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "rx_decap_mode" },
#if WLAN_PEER_DEBUG_CMD_SUPPORT
    { ATH_IOCTL_NODE_DBG_PARAM,
        IW_PRIV_TYPE_CHAR | ((IEEE80211_ADDR_LEN * 3) - 1 + 4), 
			0,  "peer_debug" },
#endif
#if WLAN_DEBUG_CMD_SUPPORT
    { ATH_IOCTL_GET_PDEV_INFO, 0,
            IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,  "pdev_info" },
    { ATH_IOCTL_GET_VDEV_INFO, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,
            IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,  "vdev_info" },
#endif
#if QCA_AIRTIME_FAIRNESS
 ...
 * @brief
 *    - iwpriv description:\n
 *         - iwpriv cmd: iwpriv radioN 6MBAck 1|0\n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN SWBcnRespT Response Time\n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN DMABcnRespT Response Time\n
 *           These settings are used to adjust the calculation of the ready
 *           time for the QoS queues.  This is used to adjust the performance
 *           of the QoS queues for optimal timing.  The parameter Software
 *           Beacon Response Time represents the time, in microseconds,
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN AggrProtDur duration \n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN AggrProtMax size\n
 *           These are used to enable RTS/CTS protection on aggregate frames,
 *           and control the size of the frames receiving RTS/CTS protection.
 *            These are typically used as a test commands to set a specific
 *           condition in the driver.  The AggrProt command is used to
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN AMPDUFrames numFrames\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN AMPDULim Byte Limit\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN ANIEna 0|1\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN AntSwap 1|0 \n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN DivtyCtl AntSel\n
 *           These commands are used to control antenna switching behavior.
 *            For 11n devices, these control which chains are used for
 *           transmit.  For Legacy devices, these are used to determine
 *           if diversity switching is enabled or disabled.  The AntSwap
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN BcnNoReset 1|0 \n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN CABlevel % Multicast \n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN CCKWeakThr 1|0\n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN chainmasksel 1|0\n
 ...
 *             - iwpriv default value: Not sure
 *             .
 *         - iwpriv cmd: iwpriv radioN CCKTrgHi High Threshold\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN CWMIgnExCCA 1|0\n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN FIRStepLvl level\n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN ForceBias Bias\n
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN ForBiasAuto 1|0\n
 *           This command activates the force bias feature.  This is
 *           used as a workaround to a directional sensitivity issue in
 *           select the bias level depending on the selected frequency.
 *            ForceBias will set the bias to a value between 0 and 7.
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN HALDbg debug level\n
 ...
 *             - iwpriv default value: Not sure
 *             .
 *         - iwpriv cmd: iwpriv radioN HTEna 1|0\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN NoiseImmLvl level\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN OFDMTrgLow Low Threshold\n
 ...
 *             - iwpriv default value: 200 errors/sec
 *             .
 *         - iwpriv cmd: iwpriv radioN OFDMTrgHi High Threshold\n
 *           These commands control the OFDM PHY Error/sec threshold settings
 *           for the ANI immunity levels.  A PHY error rate below the low
 *           trigger will cause the ANI algorithm to lower immunity thresholds,
 *           and a PHY error rate exceeding the high threshold will cause
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN OFDMWeakDet 1|0\n
 ...
 *             - iwpriv default value: Not sure
 *             .
 *         - iwpriv cmd: iwpriv radioN RSSIThrLow far threshold\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN RSSIThrHi near threshold\n
 *           These settings are used to determine the relative distance
 *           of the AP from the station.  This is used to determine how
 *           the ANI immunity levels are selected.  If the average beacon
 *           RSSI of beacons from the AP is greater than RSSIThrHi, then
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN setCountryID Country ID Num\n
 ...
 *             - iwpriv default value: Not sure
 *             .
 *         - iwpriv cmd: iwpriv radioN SpurImmLvl level\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN txchainmask mask\n
 ...
 *             .
 *         - iwpriv cmd: iwpriv radioN rxchainmask mask\n
 *           These parameters set the transmit and receive chainmask values.
 *            For MIMO devices, the chainmask indicates how many streams
 *           are transmitted/received, and which chains are used.  For
 *           some Atheros devices up to 3 chains can be used, while others
 ...
 *             - iwpriv default value: 0
 *             .
 *         - iwpriv cmd: iwpriv radioN dyntxchain 1|0\n
...
//temporary for RTT FW test. Will remove once HOST is read
#ifdef RTT_TEST
			ol_ath_rtt_meas_req(scn->wmi_handle, &scn->sc_ic);
#endif
			break;
        ...
        case OL_SPECIAL_PARAM_REGDOMAIN:
{
            ...
            chan = ieee80211_find_channel(&scn->sc_ic, freq, flags);
            if (chan) {
...
#if ATH_SUPPORT_DSCP_OVERRIDE
static int ol_ath_iw_set_dscp_tid_map(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
    struct ol_ath_softc_net80211 *scn =  ath_netdev_priv(dev);
    struct ieee80211com *ic = &scn->sc_ic;
...
static int ol_ath_iw_get_dscp_tid_map(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
    struct ol_ath_softc_net80211 *scn  = ath_netdev_priv(dev);
	struct ieee80211com *ic = &scn->sc_ic;
    ...
    tos  = (u_int8_t)param[0];
    tid  = ic->ic_dscp_tid_map[(tos >> IP_DSCP_SHIFT) & IP_DSCP_MASK];
	param[0] = (int)tid;
	return 0;
}
#endif
...
#if WLAN_DEBUG_CMD_SUPPORT
static int ol_ath_iw_get_pdev_info(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
   struct ol_ath_softc_net80211 *scn  = ath_netdev_priv(dev);

   wmi_unified_pdev_print_info(scn->wmi_handle);
}

static int ol_ath_iw_get_vdev_info(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
   struct ol_ath_softc_net80211 *scn  = ath_netdev_priv(dev);
   int *param  = (int *)extra;
   uint32_t vdev_id =  (uint32_t)param[0];

   wmi_unified_vdev_print_info(scn->wmi_handle, vdev_id);
}
#endif
...
#if ATH_SUPPORT_DSCP_OVERRIDE
	(iw_handler) ol_ath_iw_set_dscp_tid_map,  /* SIOCWFIRSTPRIV+8 */
	(iw_handler) ol_ath_iw_get_dscp_tid_map,  /* SIOCWFIRSTPRIV+9 */
#endif
#if ATH_SUPPORT_HYFI_ENHANCEMENTS
...
#endif
#if WLAN_PEER_DEBUG_CMD_SUPPORT
    (iw_handler) ol_ath_iw_node_debug_param,  /* SIOCWFIRSTPRIV+13 */
#else
NULL,
#endif
#if QCA_AIRTIME_FAIRNESS
         (iw_handler) ol_ath_iw_set_atf_sched_dur,         /* SIOCWFIRSTPRIV+14 */
#else
        NULL,
#endif
#if WLAN_DEBUG_CMD_SUPPORT
    (iw_handler) ol_ath_iw_get_pdev_info,      /* SIOCWFIRSTPRIV+15 */
    (iw_handler) ol_ath_iw_get_vdev_info,      /* SIOCWFIRSTPRIV+16 */
...
#if WLAN_PEER_DEBUG_CMD_SUPPORT
static int ol_ath_iw_node_debug_param(struct net_device *dev, 
		struct iw_request_info *info, void *w, char *extra)
{
    ...
    if(!ic ){
        printk("%s:Invalid ic param\n");
        return -EINVAL;
    }
    ni = ieee80211_find_node(&ic->ic_sta,addr);
    if(!ni){
        printk("%s:node not in NL\n");
        printk("mac:%0x:%0x:%0x:%0x:%0x:%0x,option:%d\n",addr[0],addr[1],
 			addr[2],addr[3],addr[4],addr[5],retval);
        return -EINVAL;
    }
    printk("mac:%0x:%0x:%0x:%0x:%0x:%0x,option:%d\n",addr[0],addr[1],
		addr[2],addr[3],addr[4],addr[5],retval);
    avn = OL_ATH_VAP_NET80211(ni->ni_vap);
    wmi_unified_node_set_param(scn->wmi_handle,addr,WMI_PEER_DEBUG,
                retval,avn->av_if_id);
    return 0;
}
#endif
