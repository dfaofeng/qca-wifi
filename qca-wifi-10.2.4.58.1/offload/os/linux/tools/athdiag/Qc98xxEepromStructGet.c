...
#ifdef COMMENT_4NOW
    A_UINT8 *pCalPier;
    CAL_DATA_PER_FREQ_OLPC *pCalPierStruct2G;
	CAL_DATA_PER_FREQ_OLPC *pCalPierStruct5G;

    if(ichain >= WHAL_NUM_CHAINS) {
        printf("Invalid chain index, must be less than %d\n", WHAL_NUM_CHAINS);
        return -1;
    }

    if(mode) {//5GHz
        if(ipier >= WHAL_NUM_11A_CAL_PIERS){
            printf("Invalid 5GHz cal pier index, must be less than %d\n", WHAL_NUM_11A_CAL_PIERS);
            return -1;
        }
        pCalPier = &(Qc98xxEepromStructGet()->calFreqPier5G[ipier]);
        pCalPierStruct5G = &(Qc98xxEepromStructGet()->calPierData5G[ichain][ipier]);
		*pfrequency = WHAL_FBIN2FREQ(*pCalPier, 0);
		*pcorrection = pCalPierStruct5G->refPower;
		*ptemperature = pCalPierStruct5G->thermCalVal;
		*pvoltage = pCalPierStruct5G->voltCalVal;
    } else {
        if(ipier >= WHAL_NUM_11G_CAL_PIERS){
            printf("Invalid 2GHz cal pier index, must be less than %d\n", WHAL_NUM_11G_CAL_PIERS);
            return -1;
        }

        pCalPier = &(Qc98xxEepromStructGet()->calFreqPier2G[ipier]);
        pCalPierStruct2G = &(Qc98xxEepromStructGet()->calPierData2G[ichain][ipier]);
		*pfrequency = WHAL_FBIN2FREQ(*pCalPier, 1);
		*pcorrection = pCalPierStruct2G->refPower;
		*ptemperature = pCalPierStruct2G->thermCalVal;
		*pvoltage = pCalPierStruct2G->voltCalVal;
    }
#endif
    return 0;
}
...
A_INT32 Qc98xxCalPierDataRxNoisefloorCalGet(int *value, int ix, int iy, int iz, int *num, int iBand)
{
    ...
    if (iy<0 || iy>=iMaxPier) 
    {
		if (ix<0 || ix>=WHAL_NUM_CHAINS) 
        {
			...
			for (i=0; i<WHAL_NUM_CHAINS; i++) 
            {
                ...
                {
					if (iBand==band_BG) {
						value[iv++] = Qc98xxEepromStructGet()->calPierData2G[i][j].rxNoisefloorCal;
					} else {
						value[iv++] = Qc98xxEepromStructGet()->calPierData5G[i][j].rxNoisefloorCal;
					}
				}
				*num = WHAL_NUM_CHAINS*iMaxPier;
			}
		} 
        else 
        { // get all j for ix chain
            ...
            {
				if (iBand==band_BG) {
					value[iv++] = Qc98xxEepromStructGet()->calPierData2G[ix][j].rxNoisefloorCal;
				} else {
					value[iv++] = Qc98xxEepromStructGet()->calPierData5G[ix][j].rxNoisefloorCal;
				}
			}
			*num = iMaxPier;
		}
    ...
    {
		if (iBand==band_BG) {
			value[0] = Qc98xxEepromStructGet()->calPierData2G[ix][iy].rxNoisefloorCal;
		} else {
			value[0] = Qc98xxEepromStructGet()->calPierData5G[ix][iy].rxNoisefloorCal;
		}
		*num = 1;
	}
#endif
...
A_INT32 Qc98xxCalPierDataRxNoisefloorPowerGet(int *value, int ix, int iy, int iz, int *num, int iBand)
{
    ...
    if (iy<0 || iy>=iMaxPier) 
    {
		if (ix<0 || ix>=WHAL_NUM_CHAINS) 
        {
			...
			for (i=0; i<WHAL_NUM_CHAINS; i++) 
            {
                ...
                {
					if (iBand==band_BG) {
						value[iv++] = Qc98xxEepromStructGet()->calPierData2G[i][j].rxNoisefloorPower;
					} else {
						value[iv++] = Qc98xxEepromStructGet()->calPierData5G[i][j].rxNoisefloorPower;
					}
				}
				*num = WHAL_NUM_CHAINS*iMaxPier;
			}
        ...
        { // get all j for ix chain
            ...
            {
				if (iBand==band_BG) {
					value[iv++] = Qc98xxEepromStructGet()->calPierData2G[ix][j].rxNoisefloorPower;
				} else {
					value[iv++] = Qc98xxEepromStructGet()->calPierData5G[ix][j].rxNoisefloorPower;
				}
			}
			*num = iMaxPier;
		}
    ...
    {
		if (iBand==band_BG) {
			value[0] = Qc98xxEepromStructGet()->calPierData2G[ix][iy].rxNoisefloorPower;
		} else {
			value[0] = Qc98xxEepromStructGet()->calPierData5G[ix][iy].rxNoisefloorPower;
		}
		*num = 1;
	}
#endif
...
A_INT32 Qc98xxCalPierDataRxTempMeasGet(int *value, int ix, int iy, int iz, int *num, int iBand)
{
    ...
    if (iy<0 || iy>=iMaxPier) 
    {
		if (ix<0 || ix>=WHAL_NUM_CHAINS) 
        {
			...
			for (i=0; i<WHAL_NUM_CHAINS; i++) 
            {
                ...
                {
					if (iBand==band_BG) {
						value[iv++] = Qc98xxEepromStructGet()->calPierData2G[i][j].rxTempMeas;
					} else {
						value[iv++] = Qc98xxEepromStructGet()->calPierData5G[i][j].rxTempMeas;
					}
				}
				*num = WHAL_NUM_CHAINS*iMaxPier;
			}
        ...
        { // get all j for ix chain
            ...
            {
				if (iBand==band_BG) {
					value[iv++] = Qc98xxEepromStructGet()->calPierData2G[ix][j].rxTempMeas;
				} else {
					value[iv++] = Qc98xxEepromStructGet()->calPierData5G[ix][j].rxTempMeas;
				}
			}
			*num = iMaxPier;
		}
    ...
    {
		if (iBand==band_BG) {
			value[0] = Qc98xxEepromStructGet()->calPierData2G[ix][iy].rxTempMeas;
		} else {
			value[0] = Qc98xxEepromStructGet()->calPierData5G[ix][iy].rxTempMeas;
		}
		*num = 1;
	}
#endif
...
int Qc98xxEepromCalibrationValid(void)
{
	...
	for(ic=0; ic<WHAL_NUM_CHAINS; ic++)
	{
		for(ip=0; ip<WHAL_NUM_11G_CAL_PIERS; ip++)
		{
			if(Qc98xxEepromStructGet()->calPierData2G[ic][ip].refPower!=0 && Qc98xxEepromStructGet()->calPierData2G[ic][ip].thermCalVal!=0)
			{
				return 1;
			}
		}
	...
	for(ic=0; ic<WHAL_NUM_CHAINS; ic++)
	{
		for(ip=0; ip<WHAL_NUM_11A_CAL_PIERS; ip++)
		{
			if(Qc98xxEepromStructGet()->calPierData5G[ic][ip].refPower!=0 && Qc98xxEepromStructGet()->calPierData5G[ic][ip].thermCalVal!=0)
			{
				return 1;
			}
		}
