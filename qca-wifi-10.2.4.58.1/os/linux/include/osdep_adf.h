...
#define N_WIFIPOS_SET(_nbf, _wifiposdata)   \
            (((struct ieee80211_cb *)(_nbf)->cb)->wifipos_data = (_wifiposdata))
#define N_WIFIPOS_GET(_nbf)             \
            (((struct ieee80211_cb *)(_nbf)->cb)->wifipos_data)
#define N_WIFIPOS_REQ_ID_SET(_nbf, _request_id) \
            (((struct ieee80211_cb *)(_nbf)->cb)->request_id = (_request_id))
#define N_WIFIPOS_REQ_ID_GET(_nbf) \
            (((struct ieee80211_cb *)(_nbf)->cb)->request_id)
...
#define OS_NON_SLEEPABLE()       in_softirq()
...
#define OS_SUPPORT_ASYNC_Q PARTNER_PLATFORM_ASYNC_Q
#else
...
#else
#define OS_ASYNC_Q_MAX_MESGS 32
#endif
...
struct net_device   *netdev;    /* net device handle (radio%d) */
...
#ifdef CONFIG_64BIT
u_int8_t        reserve[8]; /* workaround for wrong align access on 64-bit platform*/
#endif
...
#if defined(ATH_SUPPORT_VOWEXT) || defined(ATH_SUPPORT_AOW) ||  defined(ATH_SUPPORT_IQUE) || defined(VOW_LOGLATENCY) || (defined(UMAC_SUPPORT_NAWDS) && UMAC_SUPPORT_NAWDS != 0)

/*
 * XXX : When AoW, VoEXT, IQUE, VOW_LOGLATENCY, UMAC_SUPPORT_NAWDS is enabled along with P2P
...
#if (ATH_SUPPORT_P2P != 0) || (ATH_SUPPORT_TDLS != 0) || (ATH_SUPPORT_DEFERRED_NODE_CLEANUP != 0)
...
#endif
#endif
#if defined(UMAC_SUPPORT_SMARTANTENNA)
#if UMAC_SUPPORT_SMARTANTENNA
u_int8_t smart_antenna; /* antenna number for which training is happening */
u_int8_t rateIndex;    /*rateIndex of the current training packet packet  */
#endif
#endif
#if UMAC_SUPPORT_WNM
...
(((struct ieee80211_cb *)(_nbf)->cb)->buf_size= (_size))
#define N_BUF_SIZE_GET(_nbf) \
(((struct ieee80211_cb *)(_nbf)->cb)->buf_size)
