...
#else
#ifdef QCA_PARTNER_PLATFORM
        if( acfg_pltfrm_set_vap_param( osifp, param, value) )
            retv = -EOPNOTSUPP;
#else
        -EOPNOTSUPP;
#endif
...
#else
#ifdef QCA_PARTNER_PLATFORM
        if( acfg_pltfrm_get_vap_param( osifp, param, &value) )
#endif
		retv = EOPNOTSUPP;
#endif
    }
...
int 
acfg_vap_atf_addssid(void *ctx, a_uint16_t cmdid,
        a_uint8_t *buffer, a_int32_t Length)
{
    int rc = 0;
#if QCA_AIRTIME_FAIRNESS
    ...
    iwr.u.data.pointer = &req->data;
    iwr.u.data.length = sizeof(struct ssid_val);

    rc = ieee80211_ioctl_setatfssid(dev, &iwr);
#endif
    return rc;
}

int 
acfg_vap_atf_delssid(void *ctx, a_uint16_t cmdid,
        a_uint8_t *buffer, a_int32_t Length)
{
    int rc = 0;
#if QCA_AIRTIME_FAIRNESS
    ...
    rc = ieee80211_ioctl_delatfssid(dev, &iwr);
#endif
    return rc;
}

int 
acfg_vap_atf_addsta(void *ctx, a_uint16_t cmdid,
        a_uint8_t *buffer, a_int32_t Length)
{
    int rc = 0;
#if QCA_AIRTIME_FAIRNESS
    ...
    iwr.u.data.pointer = &req->data;
    iwr.u.data.length = sizeof(struct sta_val);

    rc = ieee80211_ioctl_setatfsta(dev, &iwr);
#endif
    return rc;
}

int 
acfg_vap_atf_delsta(void *ctx, a_uint16_t cmdid,
        a_uint8_t *buffer, a_int32_t Length)
{
    int rc = 0;
#if QCA_AIRTIME_FAIRNESS
    ...
    rc = ieee80211_ioctl_delatfsta(dev, &iwr);
#endif
    return rc;
}
