...
osif_nl_scan_evhandler(wlan_if_t vap, ieee80211_scan_event *event, void *arg)
{
    ...
    if (!nl_handle) {
        printk("%s: Scan completion (type=%d reason=%d id=%d) but on non NL "
    ...
    if (atomic_read(&nl_handle->scan_in_progress) == 0) {
        ...
        printk("%s: Scan completion id=%d (%d), but scan not active\n",
...
static void
osif_bringdown_vap_iter_obss_scan_func(void *arg, wlan_if_t vap)/* CR-765917 */
{
    int waitcnt;
...
osif_mu_report(wlan_if_t vap, struct event_data_mu_rpt *mu_rpt)
{
    ...
    WIRELESS_SEND_EVENT(dev, IWEVCUSTOM, &wreq, (void *)mu_rpt);
}
...
#endif
#if ATH_SUPPORT_WAPI
        ops.rx.wai_check = (ol_rx_check_wai_fp) osif_wai_check;
#endif
...
/* for WAPI support, ETHERTYPE_WAPI should be added to the privacy filter */
    ...
    if (ic->ic_nl_handle && cp.icp_opmode != IEEE80211_M_HOSTAP) {
        printk("Only AP mode %d is supported for NL, not %d\n", IEEE80211_M_HOSTAP, cp.icp_opmode);
        return -EINVAL;
    }
    ...
    dev = alloc_netdev(sizeof(osif_dev), name, NET_NAME_UNKNOWN, ether_setup);
    if(dev == NULL) {
...
/* CR-765917 Argument passed is  wlan_dev_t comhandle(ic) value */
...
#endif
            adf_os_sched_work((adf_os_handle_t)vap, &(wlan_vap_get_devhandle(vap))->ic_obss_scan_work);
        }
    }
}
