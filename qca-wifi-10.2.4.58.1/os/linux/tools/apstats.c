...
static int is_radio_ifname_valid(const char *ifname)
{
    ...
    if (strncmp(ifname, "radio", 5) != 0) {
        return 0;
    }

    if (!ifname[5] || !isdigit(ifname[5])) {
        return 0;
    }
	
    /* We don't make any assumptions on max no. of radio interfaces,
       at this step. */
    for (i = 6; i < IFNAMSIZ; i++)
    {
        if (!ifname[i]) {
            break;
        }
                                 ...
                                 apstats_recursion_t recursion)
{
    ...
    struct vap_user_stats vstats = {0};
    ...
    stats = (struct ieee80211_stats*)&(vstats.vap_stats);
    ucaststats = (struct ieee80211_mac_stats*)&(vstats.vap_unicast_stats);
    mcaststats = (struct ieee80211_mac_stats*)&(vstats.vap_multicast_stats); 
    ...
    vapstats->tx_err          = vapstats->tx_discard + stats->is_tx_not_ok ;
...
#else
    APSTATS_PRINT_STAT64("Tx Data Packets(valid for offload driver)",  nodestats->tx_data_packets);
    APSTATS_PRINT_STAT64("Tx Data Bytes(valid for offload driver)  ",    nodestats->tx_data_bytes);
...
#else
    APSTATS_PRINT_STAT64("Tx failures(valid for offload driver)    ",  nodestats->tx_discard);
#endif /* UMAC_SUPPORT_TXDATAPATH_NODESTATS */
