 ...
 * (default interface is radio0).  If interval is specified a rolling output
...
static void
ol_ath_stats_print(const struct wal_dbg_stats *wal_pdev_stats, FILE *fd)
{
   const struct wal_dbg_tx_stats *tx = &wal_pdev_stats->tx;
   const struct wal_dbg_rx_stats *rx = &wal_pdev_stats->rx;
    fprintf(fd,"\n### Tx ###\n");
...
static void
ol_radiostats_print(const struct ol_ath_radiostats *radio_stats, const struct wal_dbg_stats *wal_pdev_stats, FILE *fd)
{
    const struct wal_dbg_tx_stats *tx = &wal_pdev_stats->tx;
    int   i;
    ...
    fprintf(fd,"ast_tx_buf_count    :\t%lu\n",radio_stats->tx_buf_count);
    fprintf(fd,"ast_tx_packets      :\t%lu\n",radio_stats->tx_mgmt + radio_stats->tx_num_data);
    fprintf(fd,"ast_rx_packets      :\t%lu\n",radio_stats->rx_packets);
    fprintf(fd,"ast_tx_mgmt         :\t%ld\n",radio_stats->tx_mgmt);
    fprintf(fd,"ast_tx_nobuf        :\t%ld\n",tx->wmm_drop);
    fprintf(fd,"ast_tx_xretries     :\t%lu\n",tx->tx_xretry);
    fprintf(fd,"ast_rx_num_data     :\t%lu\n",radio_stats->rx_num_data);
    fprintf(fd,"ast_rx_num_mgmt     :\t%lu\n",radio_stats->rx_num_mgmt);
    fprintf(fd,"ast_rx_num_ctl      :\t%lu\n",radio_stats->rx_num_ctl);
    fprintf(fd,"ast_tx_rssi         :\t%lu\n",radio_stats->tx_rssi);
    fprintf(fd,"ast_rx_rssi_comb    :\t%lu\n",radio_stats->rx_rssi_comb);
    ...
    fprintf(fd,"tx_compaggr         :\t%lu\n",radio_stats->tx_compaggr);
    fprintf(fd,"rx_aggr             :\t%lu\n",radio_stats->rx_aggr);
    fprintf(fd,"tx_retries          :\t%d\n",tx->mpdus_requed);
    fprintf(fd,"tx_xretries         :\t%lu\n",tx->sw_retry_failure);
    fprintf(fd,"tx_bawadv           :\t%lu\n",radio_stats->tx_bawadv);
    fprintf(fd,"tx_compunaggr       :\t%lu\n",radio_stats->tx_compunaggr);
    fprintf(fd,"ast_rxorn           :\t%lu\n",radio_stats->rx_overrun);
    fprintf(fd,"ast_txurn           :\t%d\n",tx->underrun);
    fprintf(fd,"ast_txto            :\t%lu\n",tx->pdev_tx_timeout);
    fprintf(fd,"ast_mib             :\t%lu\n",radio_stats->mib_int_count);
    fprintf(fd,"ast_rx_badcrypt     :\t%lu\n",radio_stats->rx_badcrypt);
    fprintf(fd,"ast_rx_badmic       :\t%lu\n",radio_stats->rx_badmic);
#if ATH_RX_LOOPLIMIT_TIMER
