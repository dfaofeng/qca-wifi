...
static A_UINT32 freqValid(A_UINT32 val)
{
    ...
    fprintf(stderr,"Invalid channel or freq #: %d !\n", val);
...
main(int argc, char *argv[])
{
	...
	if(!strcasecmp(cmd, "start")) {
		if(argc > 3) {
			...
			fprintf(stderr,"usage: tx99tool radio0 start\n");
			goto ErrExit;
		}
	...
	/* disable */
		...
		if(argc > 3) {
			fprintf(stderr,"Too many arguments\n");
			fprintf(stderr,"usage: tx99tool radio0 stop\n");
			goto ErrExit;
		}
	...
	/* get */
	    ...
	    if(argc > 3) {
			fprintf(stderr,"Too many arguments\n");
			fprintf(stderr,"usage: tx99tool radio0 get\n");
			goto ErrExit;
		}
    ...
    /* set */
    	...
    	/* rate - Kbits/s */
    		...
    		if(argc != 5) {
    			...
    			fprintf(stderr,"usage: tx99tool radio0 set rate [Tx rate]\n");
				goto ErrExit;
    		}
    	...
    	/* Tx power */
    		...
    		if(argc != 5) {
    			fprintf(stderr,"Wrong arguments\n");
    			fprintf(stderr,"usage: tx99tool radio0 set pwr [Tx pwr]\n");
				goto ErrExit;
    		}
    	...
    	/* tx chain mask - 1, 2, 3 (for 2*2) */
    		...
    		if(argc != 5) {
    			fprintf(stderr,"Wrong arguments\n");
    			fprintf(stderr,"usage: tx99tool radio0 set txchain [Tx chain mask]\n");
				goto ErrExit;
    		}
    	...
    	/* Tx type - 0: data modulated, 1:single carrier */
    		...
    		if(argc != 5) {
    			fprintf(stderr,"Wrong arguments\n");
    			fprintf(stderr,"usage: tx99tool radio0 set type [Tx type]\n");
    			goto ErrExit;
    		}
            ...
            if(argc != 5) {
                ...
                fprintf(stderr,"usage: tx99tool radio0 set txmode [0: 11A 1: 11B  2: 11G 5: 11NAHT20 6: 11NGHT20 7: 11NAHT40PLUS 8: 11NAHT40MINUS 9: 11NGHT40PLUS 10: 11NGHT40MINUS] \n");
                goto ErrExit;
            }
            i_req.type = TX99_WCMD_SET_TXMODE;
			value = atoi(argv[4]);
		    if(!(( value >= 0) && (value <= 10) && (value !=4 ) && (value !=3 ))) {
				fprintf(stderr,"usage: tx99tool radio0 set txmode [0: 11A1: 11B 2: 11G 5: 11NAHT20 6: 11NGHT20 7: 11NAHT40PLUS 8: 11NAHT40MINUS 9: 11NGHT40PLUS 10: 11NGHT40MINUS] \n");
				goto ErrExit;
		   }
            ...
            if(argc != 5) {
                fprintf(stderr,"Wrong arguments\n");
                fprintf(stderr,"usage: tx99tool radio0 set testmode [Test Mode]\n");
                goto ErrExit;
            }
            i_req.type = TX99_WCMD_SET_TESTMODE;
			value = atoi(argv[4]);
		    if(!(( value >= 0) && ( value <= 4))) {
	            fprintf(stderr,"usage: tx99tool radio0 set testmode[0:TransMIT PN9 data 1:Transmit All zeros 2:Transmit All ones, 3:Receive only 4:Single Carrier mode\n");
				goto ErrExit;
			}
...
usage(void)
{
	fprintf(stderr, "usage: tx99tool radio0 [start|stop|set]\n");
	fprintf(stderr, "usage: tx99tool radio0 set [freq||bandwidth|htext|rate|txchain|type|pwr|txmode|testmode]\n");
	exit(-1);
}
