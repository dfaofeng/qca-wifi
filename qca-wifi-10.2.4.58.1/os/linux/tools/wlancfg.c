...
#endif
    { "HALDbg", "HALDEBUG", "NULL", "radio0", 0 },
    { "ATHDebug" , "ATHDEBUG", "NULL", "radio0", 0 },
    { "dbgLVL", "DEBUGMODE", "NULL","custom", 0 },
    { "shortgi", "SHORTGI", "NULL" , "custom", 0 },
    { "noedgech", "NO_EDGE_CH", "NULL", "custom", 1},
    { "mode", "AP_CHMODE", "NULL", "custom", 0 },
    { "set_swapled", "SWAP_LED", "NULL", "radio0", 1 },
    { "set_ledcustom", "LED_CUSTOM", "NULL", "radio0", 1 }
};

static struct iwpriv_cmds_list iwpriv_list2[] = {
    { "AMPDU", "AMPDUENABLE", "NULL", "radio0", 0 },
    { "AMPDUFrames", "AMPDUFRAMES", "NULL", "radio0", 0 },
    { "AMPDULim", "AMPDULIMIT",  "NULL", "radio0", 0 },
...
static int iwpriv_set_custom_commands_set_1(int skfd, char *apname)
{
    ...
    if (strlen(buf) && strstr(buf,"11NG")) {
        ...
        ATH_IWPRIV_SET(skfd,iwpriv_args, 2, "radio0");
    }
...
static int ath_wlan_set_country_code(int skfd)
{
    ...
    if (strlen(buf)) {
        ...
        ret = iwpriv_set_private(skfd, iwpriv_args, 2, "radio0");

        if (ret != 0) {
            fprintf(stderr,"%s: Error setting iwpriv var : %s \
                                value : %s ap : %s\n", \
                                __func__, iwpriv_args[0], \
                                 iwpriv_args[1], "radio0");
        }
        else
...
static int makeVAP(char *vapmode, char *ifmode, char *apname, int staflag, int unit)
{
        ...
        if (iwpriv_init(skfd, "radio0") != 0)
            goto makevap_cleanup;

        if (ath_wlan_set_country_code(skfd) != 0)
            goto makevap_cleanup;
	
        ret = wlan_create_interface(skfd, apname, "radio0", vapmode, staflag);
        ...
        ret = athcfg_ifconfig_ops("radio0", "txqueuelen", buf);
        ...
        //fprintf(stdout,"%s: iwpriv_init for radio0 and %s success\n", __func__, apname);
