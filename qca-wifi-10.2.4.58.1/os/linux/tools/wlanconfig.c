...
/*
 * wlanconfig athX create wlandev radioX
...
static int getopmode(const char *, u_int16_t *);
        ...
        if (argc >= 2) {
            if (streq(argv[2], "addssid"))
            {
                ...
                if(argc != 5)
                {
                    ...
                    set_addsta_pval(ifname,argv[3],argv[4]);
                }
...
usage(void)
{
	fprintf(stderr, "usage: wlanconfig athX create wlandev radioX\n");
    fprintf(stderr, "                  wlanmode [sta|adhoc|ap|monitor|wrap|p2pgo|p2pcli|p2pdev]\n"
...
#endif
#ifdef ATH_BUS_PM
	fprintf(stderr, "usage: wlanconfig radioX suspend|resume\n");
#endif
#if QCA_AIRTIME_FAIRNESS
...
static int
getopmode(const char *s, u_int16_t *flags)
{
	if (streq(s, "sta"))
...
#define LIST_STATION_ALLOC_SIZE 24*1024
	...
	do {
		...
		printf("%s %4u %4d %3dM %6dM %4d %4d %6d %7d %5.4s %-5.5s %3x %10x %14d %14.6s %02u:%02u:%02u"
			...
			char *addr1, char *addr2)
{
    ...
    struct iwreq iwr;
    struct ieee80211_wlanconfig *config;
    char wds_ni_macaddr[17], wds_macaddr[17], data[4096];
    ...
    if (addr1) {
        strncpy(wds_ni_macaddr, addr1, 17);
