...
wlan_set_desired_ibsschan(wlan_if_t vaphandle, int channum)
{
    ...
    if(channum == IEEE80211_CHAN_ANY) {
        ...
        channel = ieee80211_find_dot11_channel(ic, channum, vap->iv_des_mode | ic->ic_chanbwflag);
        if (channel == NULL) {
            channel = ieee80211_find_dot11_channel(ic, channum, IEEE80211_MODE_AUTO);
            if (channel == NULL)
                return -EINVAL;
        }
    }
        ...
        struct ieee80211_channel_list *chan_info)
{
    ...
    if (IEEE80211_IS_CHAN_11AC_VHT40PLUS(chan) ||
        ...
        chan_info->cl_channels[1] = ic->ic_find_channel(ic, chan->ic_freq + 20, IEEE80211_CHAN_11NA_HT20);
    } else if (IEEE80211_IS_CHAN_11AC_VHT40MINUS(chan) ||
...
void    
ieee80211_get_extchan_info(struct ieee80211com *ic, 
        struct ieee80211_channel_list *chan_info)
{
    ieee80211_get_extchaninfo(ic, ic->ic_curchan, chan_info);
    return;
}
