...
wep_decap(struct ieee80211_key *k, wbuf_t wbuf, int hdrlen, struct ieee80211_rx_status *rs)
{
    ...
    if (rs->rs_flags & IEEE80211_RX_DECRYPT_ERROR) {
        ...
        IEEE80211_NODE_STAT_ADDRBASED(vap, wh->i_addr2, rx_wepfail);
        return 0;
    }
...
#define S_SWAP(a,b) do { uint8_t t = S[a]; S[a] = S[b]; S[b] = t; } while(0)
    ...
    if (wbuf->mspd_data) {
        origHdr = (u_int8_t *)wbuf->mspd_data + wbuf->mspd_ofst -sizeof(struct ieee80211_qosframe) - ATH_COMCERTO_CUSTOM_SKB_OFFSET;
    }
    else
#endif
...
#ifdef CONFIG_COMCERTO_CUSTOM_SKB_LAYOUT
    if (wbuf->mspd_data) {
        pos = (u_int8_t *)wbuf->mspd_data + wbuf->mspd_ofst -sizeof(struct ieee80211_qosframe) - ATH_COMCERTO_CUSTOM_SKB_OFFSET;
    }
    else
#endif        
