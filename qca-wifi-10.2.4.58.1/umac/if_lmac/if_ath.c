...
static int ath_setup_wrap_key(struct ieee80211vap *vap, const u_int8_t *mac,
                              struct ieee80211_key *k);
static int ath_setup_bcast_mac_entry(struct ieee80211vap *vap, struct ieee80211_key *k);
#endif
...
static void ath_net80211_print_keyerror(struct ieee80211com *ic, char *mac, char *msg);
#endif
#if UNIFIED_SMARTANTENNA
static void ath_net80211_smart_ant_enable(struct ieee80211com *ic, int enable, int mode, uint32_t rx_antenna);
...
static void ath_net80211_smart_ant_prepare_rateset(struct ieee80211com *ic, struct ieee80211_node *ni);
...
#define ATH_NSCAN_PSTA_VAPS 5
    ...
    if (avn->av_is_wrap || avn->av_is_psta) {
        ...
        if (avn->av_is_wrap) {
            ...
            avn->av_bcast_key.wk_keyix = IEEE80211_KEYIX_NONE;
        } else { /* avn->av_is_psta */
            if (avn->av_is_mpsta) {
                /*
...
#define PROXYSTA_HW_CRYPT_MIN_DELIMS 5
     ...
     */
    if (ni && (ni->ni_flags & IEEE80211_NODE_EXTRADELIMWAR))
        return 2;
...
static int ath_vap_up(struct ieee80211vap *vap)
{
    ...
    case IEEE80211_M_IBSS:
        ...
        if (avn->av_is_wrap) {
			...
			if(avn->av_bcast_key.wk_keyix != IEEE80211_KEYIX_NONE) {
				scn->sc_ops->key_delete(scn->sc_dev, avn->av_bcast_key.wk_keyix, 1);
			}
			error = ath_setup_bcast_mac_entry(vap, &avn->av_bcast_key);
			IEEE80211_DPRINTF(vap, IEEE80211_MSG_WRAP,"%s: create key entry with bcast mac addr for vap %s %s\n", __func__, ether_sprintf(vap->iv_myaddr), error ? "succeeded" : "failed");
               ...
               struct ieee80211_node *ninfo)
{
    ...
    if (avn->av_is_psta && k != &avn->av_psta_key &&
        (keyix == avn->av_psta_key.wk_keyix ||
         ((WRAP_HW_DECRYPT_PSTA_WEP || sc->sc_wrap_hwcrypt) &&
            ...
            const u_int8_t bssid[IEEE80211_ADDR_LEN], struct ieee80211_node *ni)
{
    ...
    ni = ieee80211_vap_find_node(vap, bssid);
            ...
            int is_proxy_addr)
{
...
#if ATH_SUPPORT_WRAP
	if (is_proxy_addr) {
		if ((WRAP_HW_DECRYPT_PSTA_WEP || sc->sc_wrap_hwcrypt) && key_type == HAL_CIPHER_WEP) {
			struct ath_vap_net80211 *avn = ATH_VAP_NET80211(vap);
	...
	if (WRAP_HW_ENCRYPT_WRAP_WEP || sc->sc_wrap_hwcrypt) {
        struct ath_vap_net80211 *avn = ATH_VAP_NET80211(vap);
...
static void ath_net80211_print_keyerror(struct ieee80211com *ic, char *node_mac, char *msg)
{
    ieee80211_print_keyerror(ic,node_mac,msg);
    return;
}
#endif
            ...
            const u_int8_t peermac[IEEE80211_ADDR_LEN])
{
    ...
    if (avn->av_is_wrap) {
         ...
         */
       if ((k->wk_flags & IEEE80211_KEY_GROUP &&
             k->wk_cipher->ic_cipher != IEEE80211_CIPHER_WEP && !sc->sc_wrap_hwcrypt) ||
            (!(WRAP_HW_ENCRYPT_WRAP_WEP || sc->sc_wrap_hwcrypt) && k->wk_cipher->ic_cipher == IEEE80211_CIPHER_WEP) ||
        ...
        int ret;
        /*
		 * In azimuth mode, to receive unicast packets from RootAP,
		 * plumb key on keycache entry which has proxy-sta vap mac address.
		 */
		if ((WRAP_HW_DECRYPT_PSTA_WEP || sc->sc_wrap_hwcrypt) && (k->wk_cipher->ic_cipher == IEEE80211_CIPHER_WEP))
		{
			memcpy(avn->av_psta_key.wk_key, k->wk_key, sizeof(k->wk_key));
			avn->av_psta_key.wk_cipher = k->wk_cipher;
       ...
       if ((k->wk_flags & IEEE80211_KEY_GROUP &&
             k->wk_cipher->ic_cipher != IEEE80211_CIPHER_WEP && !sc->sc_wrap_hwcrypt) ||
            (!(WRAP_HW_DECRYPT_PSTA_WEP || sc->sc_wrap_hwcrypt) && k->wk_cipher->ic_cipher == IEEE80211_CIPHER_WEP) ||
	   ...
	   if (!(sc->sc_wrap_hwcrypt && (k->wk_flags & IEEE80211_KEY_GROUP))) {
                   ...
                   struct ieee80211_key *k)
{
     ...
     */
	return __ath_key_set(vap, k, mac, 1);
}

static int
ath_setup_bcast_mac_entry(struct ieee80211vap *vap, struct ieee80211_key *key)
{
	struct ieee80211com *ic = vap->iv_ic;
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ath_softc *sc = scn->sc_dev;
	...
	if(ath_hal_getcapability(sc->sc_ah, HAL_CAP_WRAP_PROMISC, 0, NULL) == HAL_ENOTSUPP) {
		...
		key->wk_keyix = keyix;
		return __ath_key_set(vap, key, bmac, 0);
	}
	return 0;
}
#endif
...
static void
ath_bsteering_rssi_update(ieee80211_handle_t ieee, u_int8_t *sta_mac_addr, u_int8_t tx_status,int8_t rssi)
{
    struct ieee80211com *ic = NET80211_HANDLE(ieee);
...
#if ATH_SUPPORT_HYFI_ENHANCEMENTS
void ath_net80211_get_ald_statistics(struct ieee80211vap *vap)
{
    struct ieee80211com *ic = vap->iv_ic;
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ieee80211_node_table *nt = &ic->ic_sta;
...
static void ath_net80211_rate_newstate(ieee80211_handle_t ieee, ieee80211_if_t if_data)
{
    ...
    if (ieee80211vap_get_opmode(vap) != IEEE80211_M_STA) {
        ...
        if((vap->iv_bsschan == NULL) ||
            ...
            printk("VAP channel is not set for vap %p investigate!\n", vap);
            return;
        }
        /*
...
struct ieee80211_ops net80211_ops = {
   ...
   NULL,                                   /* ath_net80211_rxcleanup */
    ...
    ath_net80211_acs_get_param,               /* set acs_param*/
#if ATH_SUPPORT_KEYPLUMB_WAR
...
static void ath_net80211_atf_set( struct ieee80211com *ic, u_int8_t enable)
{
    ...
    scn->sc_ops->ath_atf_set(scn->sc_dev, enable);
}

static void ath_net80211_atf_clear( struct ieee80211com *ic, u_int8_t disable)
{
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
...
#ifdef ATH_SUPPORT_MSI
    rc = ath_init(&scn->sc_ic);

    /*
     * Start MSI poll timer if MSI is enabled.
     */
...
#ifdef ATH_SUPPORT_MSI
    /*
     * Stop MSI poll timer if MSI is enabled.
     */
    if (bus_msi_enabled(scn->sc_osdev)) {
...
int ath_net80211_add_hmmc(struct ieee80211vap *vap, u_int32_t ip, u_int32_t mask)
{
    ...
    if (!ic || !ip || !mask ||
            ((htobe32(ip) & htobe32(0xf0000000)) != htobe32(0xe0000000)))
        return -EINVAL;
	...
	ieee80211_enumerate_channels(chan, ic, i) {
		...
		if (nol_found && (! IEEE80211_IS_CHAN_RADAR(chan)))
			adf_os_print("%s: radar found = chan=%p, freq=%d, "
			    ...
			    chan,
			    chan->ic_freq,
			    ieee80211_ieee2mhz(ic, chan->ic_vhtop_ch_freq_seg1,
			        chan->ic_flags),
			    chan->ic_flags);
...
static void ath_net80211_smart_ant_prepare_rateset(struct ieee80211com *ic, struct ieee80211_node *ni)
{
    ...
    if (scn->sc_ops->smart_ant_prepare_rateset) {
        scn->sc_ops->smart_ant_prepare_rateset(scn->sc_dev, an->an_sta, &ni->rate_info);
    }
}
