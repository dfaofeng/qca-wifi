...
ieee80211_add_htcap_cmn(struct ieee80211_node *ni, struct ieee80211_ie_htcap_cmn *ie, u_int8_t subtype)
{
     ...
     */
    htcap = ic->ic_htcap;
    htcap &= (((ic->ic_htflags & IEEE80211_HTF_SHORTGI40) && vap->iv_sgi) ?
                                 ic->ic_htcap  : ~IEEE80211_HTCAP_C_SHORTGI40);
    htcap &= (((ic->ic_htflags & IEEE80211_HTF_SHORTGI20) && vap->iv_sgi) ?
...
ieee80211_parse_htcap(struct ieee80211_node *ni, u_int8_t *ie)
{
    ...
    if (!IEEE80211_IS_TDLS_NODE(ni)) {
        if (ni->ni_htcap & IEEE80211_HTCAP_C_SHORTGI40)
            ni->ni_htcap  = ni->ni_htcap & (((ic->ic_htflags & IEEE80211_HTF_SHORTGI40) && vap->iv_sgi)
                                        ? ni->ni_htcap  : ~IEEE80211_HTCAP_C_SHORTGI40);
        if (ni->ni_htcap & IEEE80211_HTCAP_C_SHORTGI20)
            ni->ni_htcap  = ni->ni_htcap & (((ic->ic_htflags & IEEE80211_HTF_SHORTGI20) && vap->iv_sgi)
...
ieee80211_parse_vhtcap(struct ieee80211_node *ni, u_int8_t *ie)
{
    ...
    if (ni->ni_vhtcap & IEEE80211_VHTCAP_RX_LDPC) {
        ni->ni_vhtcap  = ni->ni_vhtcap & ((vap->iv_ldpc & IEEE80211_HTCAP_C_LDPC_RX) ? ni->ni_vhtcap  : ~IEEE80211_VHTCAP_RX_LDPC);
    }
...
ieee80211_set_vht_rates(struct ieee80211com *ic, struct ieee80211vap  *vap)
{
    ...
    switch (tx_streams) {
        ...
        case 1:
            ...
            if(bcc_11ng_256qam_20mhz_s) {
                 ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = VHT_MCSMAP_NSS1_MCS0_8; /* MCS 0-8 */
            }
            else {
                 ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = VHT_MCSMAP_NSS1_MCS0_9; /* MCS 0-9 */
            }
            if (vap->iv_vht_tx_mcsmap) {
                ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = (vap->iv_vht_tx_mcsmap | VHT_MCSMAP_NSS1_MASK); 
            }
        break;
        ...
        case 2:
            /* Dual stream */
             /*MCS9 is not supported for BCC, NSS=1,2 in 20Mhz */
            if(bcc_11ng_256qam_20mhz_s) {
                 ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = VHT_MCSMAP_NSS2_MCS0_8; /* MCS 0-8 */
            }
            else {
                 ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = VHT_MCSMAP_NSS2_MCS0_9; /* MCS 0-9 */
            }
            if (vap->iv_vht_tx_mcsmap) {
                ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = (vap->iv_vht_tx_mcsmap | VHT_MCSMAP_NSS2_MASK); 
            }
        break;
        ...
        case 3:
            ...
            if (vap->iv_vht_tx_mcsmap) {
                ic->ic_vhtcap_max_mcs.tx_mcs_set.mcs_map = (vap->iv_vht_tx_mcsmap | VHT_MCSMAP_NSS3_MASK); 
            }
        break;
    }
    ...
    switch (rx_streams) {
        ...
        case 1:
             /*MCS9 is not supported for BCC, NSS=1,2 in 20Mhz */
            if(bcc_11ng_256qam_20mhz_s) {
                 ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = VHT_MCSMAP_NSS1_MCS0_8; /* MCS 0-8 */
            }
            else {
                 ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = VHT_MCSMAP_NSS1_MCS0_9;
            }
            if (vap->iv_vht_rx_mcsmap) {
                ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = (vap->iv_vht_rx_mcsmap | VHT_MCSMAP_NSS1_MASK); 
            }
        break;
        ...
        case 2:
            /* Dual stream */
             /*MCS9 is not supported for BCC, NSS=1,2 in 20Mhz */
            if(bcc_11ng_256qam_20mhz_s) {
                 ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = VHT_MCSMAP_NSS2_MCS0_8; /* MCS 0-8 */
            }
            else {
                 ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = VHT_MCSMAP_NSS2_MCS0_9;
            }
            if (vap->iv_vht_rx_mcsmap) {
                ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = (vap->iv_vht_rx_mcsmap | VHT_MCSMAP_NSS2_MASK); 
            }
        break;
        ...
        case 3:
            ...
            if (vap->iv_vht_rx_mcsmap) {
                ic->ic_vhtcap_max_mcs.rx_mcs_set.mcs_map = (vap->iv_vht_rx_mcsmap | VHT_MCSMAP_NSS3_MASK); 
            }
        break;
    }
}
                     ...
                     struct ieee80211com *ic, u_int8_t subtype)
{
    ...
    u_int32_t vendor_oui= ((VHT_INTEROP_OUI << 8) | (VHT_INTEROP_TYPE));
    ...
    /* Fill in the VHT capabilities info */
    memcpy(&vht_interopcap->vht_interop_oui,&vendor_oui,4);
    vht_interopcap->sub_type=VHT_INTEROP_OUI_SUBTYPE;
    ieee80211_add_vhtcap(frm + 7 , ni, ic, subtype); /* Vht IE location Inside Vendor specific VHT IE*/


    if ((subtype == IEEE80211_FC0_SUBTYPE_BEACON)||(subtype == IEEE80211_FC0_SUBTYPE_ASSOC_RESP) || 
                                                   (subtype == IEEE80211_FC0_SUBTYPE_REASSOC_RESP)) {
       ieee80211_add_vhtop(frm + 21 , ni, ic, subtype); /* Adding Vht Op IE after Vht cap IE  inside Vendor VHT IE*/       
    }
